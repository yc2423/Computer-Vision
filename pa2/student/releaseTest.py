...
class TestSegment(unittest.TestCase):
    ...
    def test_grad(self):
        ...
        #print max(np.absolute((maggrad_exp - maggrad).flatten()))
    ...
    def test_kmeans_integrated(self):
        img, k, randseed, exp_out, exp_niter = self.data['B_kint']
        np.random.seed(randseed)
        res_out, res_niter = segment.kMeansSegmentation(img, k)
        self.assertTrue(np.allclose(res_out, exp_out, atol=EPSILON))
    ...
    def test_ncut_color_weight(self):
        ...
        #print np.max(np.absolute((exp_out - res_out).flatten()))
        self.assertTrue(np.allclose(exp_out, res_out, atol=EPSILON))
