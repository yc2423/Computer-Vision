...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        dy_2 = dy**2
        dxy = dx*dy
        gdx_2 = scipy.ndimage.gaussian_filter(dx_2,0.5)
        gdy_2 = ndimage.gaussian_filter(dy_2,sigma=0.5)
        gdxy = ndimage.gaussian_filter(dxy,sigma=0.5)

        harrisImage = gdx_2*gdy_2 - gdxy*gdxy - 0.1*(gdx_2 + gdy_2)**2
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        # TODO-BLOCK-BEGIN
        destImage2 = ndimage.filters.maximum_filter(harrisImage,size=7)
        destImage = harrisImage == destImage2
        # TODO-BLOCK-END
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        pad_img = np.pad(grayImage,2,mode='constant')
        ...
        for i, f in enumerate(keypoints):
            ...
            x+=2
            y+=2
            desc[i] = pad_img[y-2:y+3,x-2:x+3].flatten()
            #raise Exception("TODO in features.py not implemented")
...
class MOPSFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            trs_mx = transformations.get_trans_mx(np.array([-f.pt[0],-f.pt[1],0]))
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        for i in range(0,desc1.shape[0]):
            idx = np.argmin(dist[i,:])
            distance1 = np.min(dist[i,:])
            dist[i,idx] = sys.maxint
            distance2 = np.min(dist[i,:])
            matches.append(cv2.DMatch(i,idx,distance1/distance2))
